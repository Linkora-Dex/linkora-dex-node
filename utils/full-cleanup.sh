...
echo "🧹 Full cleanup: containers, images, volumes for project..."
...
CONTAINERS=$(docker ps -a --filter "name=${PROJECT_PREFIX}" -q)
...
IMAGES=$(docker images --filter "reference=${PROJECT_PREFIX}*" -q)
if [ ! -z "$IMAGES" ]; then
   docker rmi -f $IMAGES
...
NETWORK="${PROJECT_PREFIX}_anvil-network"
